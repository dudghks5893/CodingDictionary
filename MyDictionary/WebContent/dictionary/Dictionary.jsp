...
<body>
		...
		<%
			if(search!=null){
		%>
			<a href="Dictionary.jsp?search=<%=search%>&pageNum=<%=back%>" class="btn btn-outline-info mr-2"><b>&laquo;</b></a>
		<%
			} else{
		%>
			<a href="Dictionary.jsp?pageNum=<%=back%>" class="btn btn-outline-info mr-2"><b>&laquo;</b></a>
		<%
			}
			for(int p=startBlock; p<=block; p++){
				if(search==null){
					if(pageNum==p){
		%>
			<a href="Dictionary.jsp?pageNum=<%=p%>" class="btn btn-info"><%=p%></a>
		<%		
					} else{
						
		%>
			<a href="Dictionary.jsp?pageNum=<%=p%>" class="btn btn-outline-info"><%=p%></a>
		<% 				
					}
				} if(search!=null){
					if(pageNum==p){
		%>
			<a href="Dictionary.jsp?search=<%=search%>&pageNum=<%=p%>" class="btn btn-info"><%=p%></a>
		<%			
					} else{
		%>
			<a href="Dictionary.jsp?search=<%=search%>&pageNum=<%=p%>" class="btn btn-outline-info"><%=p%></a>
		<%
					}
				}
			}
		...
		<%
			if(search!=null){
		%>
			<a href="Dictionary.jsp?search=<%=search%>&pageNum=<%=next%>" class="btn btn-outline-info ml-2"><b>&raquo;</b></a>
		<%
			} else{
		%>
			<a href="Dictionary.jsp?pageNum=<%=next%>" class="btn btn-outline-info ml-2"><b>&raquo;</b></a>
		<%
			}
		%>
			</div>
