...
<body>
		...
		%>	
					<tr>
						...
						<td><a href="./updatePage.jsp?num=<%=list.get(i).getNum()%>" class="btn btn-primary" style="height: 25px; padding: 1px; color: white;" >코드수정 &raquo;</a></td>
					</tr>
		<%
			}
		%>
		...
		<%
			if(search!=null){
		%>
			<a href="updateDictionary.jsp?search=<%=search%>&pageNum=<%=back%>" class="btn btn-outline-info mr-2"><b>&laquo;</b></a>
		<%
			} else{
		%>
			<a href="updateDictionary.jsp?pageNum=<%=back%>" class="btn btn-outline-info mr-2"><b>&laquo;</b></a>
		<%
			}
			for(int p=startBlock; p<=block; p++){
				if(search==null){
					if(pageNum==p){
		%>
			<a href="updateDictionary.jsp?pageNum=<%=p%>" class="btn btn-info"><%=p%></a>
		<%		
					} else{
						
		%>
			<a href="updateDictionary.jsp?pageNum=<%=p%>" class="btn btn-outline-info"><%=p%></a>
		<% 				
					}
				} if(search!=null){
					if(pageNum==p){
		%>
			<a href="updateDictionary.jsp?search=<%=search%>&pageNum=<%=p%>" class="btn btn-info"><%=p%></a>
		<%			
					} else{
		%>
			<a href="updateDictionary.jsp?search=<%=search%>&pageNum=<%=p%>" class="btn btn-outline-info"><%=p%></a>
		<%
					}
				}
			}
		...
		<%
			if(search!=null){
		%>
			<a href="updateDictionary.jsp?search=<%=search%>&pageNum=<%=next%>" class="btn btn-outline-info ml-2"><b>&raquo;</b></a>
		<%
			} else{
		%>
			<a href="updateDictionary.jsp?pageNum=<%=next%>" class="btn btn-outline-info ml-2"><b>&raquo;</b></a>
		<%
			}
		%>
			</div>		
